...
function render_programtime_list($program = [], $error = "") {
...
function render_programtime_card($program_id, $error = "") {
    if (!is_numeric($program_id)) {
        ...
        $program = get_programtime_by_id($program_id);
        if ($program == []) {
            echo "<h4>Engin æfing er til með þetta #</h4>";
            echo "<h4>Smelltu <a href=".esc_url(CF550_ADMIN_URL.'cf550_programs').">hér</a> til að fara til baka</h4>";
            return;
...
function update_programtime() {
    if (!isset($_POST["program_id"])) {
        render_programtime_card([], "default-error");
        return;
    }
    $id = $_POST["program_id"];
    ...
    if (preg_match("/^\d{4}-\d{2}-\d{2}$/", $date) == false) {
        render_programtime_card($id, "Dagsetning þarf að vera á forminu: ÁÁÁÁ-MM-DD (t.d. 2016-02-28 fyrir tuttugasta og áttunda febrúar 2016).");
        return;
    }
    ...
    if ($query_response === false) { // error
        render_programtime_list(["date" => $date, "title" => $title, "description" => $description], "default-error");
        return;
    }
